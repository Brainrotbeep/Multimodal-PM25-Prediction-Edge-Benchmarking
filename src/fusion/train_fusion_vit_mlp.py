    ...
    [
        {
            "params": [
                ...
                if "vit.blocks" in n and p.requires_grad
            ],
            "lr": 1e-5
        },
        {
            "params": [
                p for n, p in model.named_parameters()
                if "vit.blocks" not in n and p.requires_grad
            ],
            "lr": 3e-4
        }
    ],
...
for epoch in range(epochs):
    ...
    if (epoch + 1) % 10 == 0:
        ...
        torch.save(
            model.state_dict(),
            f"models/fusion_vit_mlp_epoch_{epoch+1}.pth"
        )
